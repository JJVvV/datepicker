...
var RowRenderer = Class.extend({
	...
	//alex
	rowWeek: function(type, row){
		var col,cell, rowCellHtml = '';
		for(col = 0; col < this.colCnt; col++){
			rowCellHtml += '<th class="fc-day-header fc-widget-header fc-sun">'+ dayIDs[col] +'</th>';
		}
		rowCellHtml = this.bookendCells(rowCellHtml, type, row); // apply intro and outro

		return '<tr class="newmeeting-calendar-week">' + rowCellHtml + '</tr>';
	},
	...
	headHtml: function() {
		return '' +
			'<div class="fc-row ' + this.view.widgetHeaderClass + '">' +
				'<table>' +
					'<thead>' +
			(this.view.options.weekOnTop ? this.rowWeek('head', 0) : '')+
	...
	getEventTimeText: function(range, formatStr, displayEnd) {
		...
		if (this.displayEventTime && range.start.hasTime()) {
			if (displayEnd && range.end) {
				return this.view.formatRange(range, formatStr, this.view.opt('timerSeparator'));
			}
			else {
