/*获取图片的真实地址*/
export function ImagePath(code) {
    let emojiImg = "/redux-launchr/public/img/emoji/";
    return [emojiImg, code, '.png'].join('');
};
/*显示的表情地址*/
export let EmojiItems = (()=> {
    return [{
        unicode: '1f600',
        code: "\uD83D\uDE00"
    }, {
...
let emoji = {
    ...
    map: {},
    init(){
        if (this.inits.unified) return;
        this.inits.unified = 1;
        var a = [];
        this.map.unified = {};
        for (var i in this.data) {
            for (var j = 0; j < this.data[i][0].length; j++) {
                a.push(this.data[i][0][j]);
                this.map.unified[this.data[i][0][j]] = i;
            }
        }
        this.rx_unified = new RegExp('(' + a.join('|') + ')', "g");
    },
    replacement(val){
        return "<img class='emoji' src='"+ImagePath(val)+"' />";
    }
};
export function ReplaceUnified(str) {
    emoji.init();
